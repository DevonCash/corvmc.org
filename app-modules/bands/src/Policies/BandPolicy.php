...
namespace CorvMC\Bands\Policies;
...
class BandPolicy
{
    ...
    public function contact(?User $user, Band $band): ?bool
    {
        ...
        if ($contact->visibility === \CorvMC\Moderation\Enums\Visibility::Public) {
            return true;
        }

        // Members-only bands: users with permission can view
        if ($contact->visibility === \CorvMC\Moderation\Enums\Visibility::Members && $user) {
            return true;
        }

        // Private bands: band members can view contact info
        if ($contact->visibility === \CorvMC\Moderation\Enums\Visibility::Private && $band->membership($user)) {
            return true;
        }
