...
class RehearsalReminderNotification extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject("Reminder: Rehearsal on {$this->date} — are you going?")
            ->greeting('Hello!')
            ->line("Just a reminder — {$this->bookerName} has a rehearsal coming up and is still waiting to hear from you.")
            ->line("**When:** {$this->date}")
            ->line("**Time:** {$this->time}")
    ...
    public function toDatabase(object $notifiable): array
    {
        return [
            ...
            'body' => "Reminder: rehearsal on {$this->date} at {$this->time}. Can you make it?",
