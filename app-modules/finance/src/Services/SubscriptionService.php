...
class SubscriptionService
{
     ...
     * 
     * @return SubscriptionStatsData Subscription statistics
     */
    public function getSubscriptionStats(): SubscriptionStatsData
    {
        return Cache::remember('subscription_stats', 1800, function () {
            ...
            $totalAllocatedHours = $this->getSustainingMembers()
                ->sum(fn ($user) => app(MemberBenefitService::class)->getUserMonthlyFreeHours($user));
            ...
            return new SubscriptionStatsData(
                ...
                mrr_base: $mrrBase,
                mrr_total: $mrrTotal,
                average_contribution: $averageMrr,
                median_contribution: $medianContribution,
                new_members_this_month: $newMembersThisMonth,
                subscription_net_change: $subscriptionNetChange,
     ...
     * Get subscription amounts from Stripe.
     * 
     * @param Collection $subscriptions Collection of subscriptions
     * @return Collection Collection of subscription amounts
     */
    protected function getSubscriptionAmountsFromStripe(Collection $subscriptions): Collection
    {
    ...
    /**
     * Calculate median contribution amount.
     * 
     * @param Collection $subscriptionAmounts Collection of subscription amounts
     * @return Money The median contribution
     */
    protected function calculateMedianContribution(Collection $subscriptionAmounts): Money
    {
        if ($subscriptionAmounts->isEmpty()) {
            return Money::zero('USD');
        }

        $amounts = $subscriptionAmounts->pluck('amount')->sort()->values();
        $count = $amounts->count();
        
        if ($count % 2 === 0) {
            // Even number of items - average the two middle values
            $median = ($amounts->get($count / 2 - 1) + $amounts->get($count / 2)) / 2;
        } else {
            // Odd number of items - take the middle value
            $median = $amounts->get(floor($count / 2));
        }
        
        return Money::ofMinor((int) round($median), 'USD');
    }
}