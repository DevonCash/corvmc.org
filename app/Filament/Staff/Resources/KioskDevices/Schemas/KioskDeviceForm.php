...
class KioskDeviceForm
{
    ...
    protected static function apiKeySection(): Section
    {
        return Section::make('API Key')
            ...
            ->collapsible()
            ->schema([
                TextEntry::make('api_key_display')
                    ->label('API Key')
                    ->state(fn (?KioskDevice $record) => $record?->api_key ?? 'Will be generated on save')
                    ->helperText('Copy this key to configure the kiosk app. It cannot be changed after creation.')
                    ->columnSpanFull(),
                TextEntry::make('last_seen_at')
                    ->label('Last Activity')
                    ->state(fn (?KioskDevice $record) => $record?->last_seen_at?->diffForHumans() ?? 'Never')
                    ->columnSpanFull(),
            ]);
    }
    ...
    protected static function capabilitiesSummary(): TextEntry
    {
        return TextEntry::make('capabilities_summary')
            ->label('Available Features')
            ->state(function ($get, ?KioskDevice $record) {
