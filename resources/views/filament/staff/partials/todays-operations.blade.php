    ...
    <div class="p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            @if ($data['space_status']['is_open'])
                <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700 ring-1 ring-inset ring-green-600/20 dark:bg-green-400/10 dark:text-green-400 dark:ring-green-400/20">
                    <span class="h-1.5 w-1.5 rounded-full bg-green-600 dark:bg-green-400"></span>
                    Space Open
                </span>
            @else
                <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-red-50 px-3 py-1 text-sm font-medium text-red-700 ring-1 ring-inset ring-red-600/20 dark:bg-red-400/10 dark:text-red-400 dark:ring-red-400/20"
                    title="{{ $data['space_status']['reason'] }}">
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('filament.staff.resources.space-management.index') }}"
                        class="text-xs text-cyan-600 hover:text-cyan-500 dark:text-cyan-400">
                        View all
                    </a>
                </div>
                ...
                @if ($data['reservations']->isEmpty())
                    <div
                        class="flex flex-col items-center justify-center py-8 text-center rounded-lg bg-gray-50 dark:bg-gray-800/50">
                ...
                @else
                    <div class="space-y-2 max-h-64 overflow-y-auto">
                        @foreach ($data['reservations'] as $reservation)
                            <a href="{{ route('filament.staff.resources.space-management.view', $reservation['id']) }}"
                                class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800/70 transition-colors">
                                <div class="flex items-center gap-3 min-w-0 flex-1">
                                    <div
                                        class="flex flex-col items-center justify-center px-2 py-1 bg-white dark:bg-gray-900 rounded border border-gray-200 dark:border-gray-700 min-w-[52px]">
                                        <div class="text-xs font-medium text-gray-600 dark:text-gray-400">
                                            {{ $reservation->getStartTime()->format('g:i A') }}
                                        </div>
                                        <div class="text-[10px] text-gray-400 dark:text-gray-500">
                                            {{ $reservation->getEndTime()->format('g:i A')}}
                                        </div>
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white truncate">
                                            {{ $reservation->getDisplayTitle() }}
                                        </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    @if ($data['tonights_event'])
                        <a href="{{ route('filament.staff.resources.events.edit', $data['tonights_event']) }}"
                            ...
                            <div class="flex items-start gap-3">
                                <div
                                    class="flex h-10 w-10 items-center justify-center rounded-lg bg-cyan-100 dark:bg-cyan-900/30 flex-shrink-0">
                    ...
                    @else
                        <div
                            class="flex flex-col items-center justify-center py-6 text-center rounded-lg bg-gray-50 dark:bg-gray-800/50">
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        @if ($data['checked_out_equipment']->isNotEmpty())
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ $data['checked_out_equipment']->count() }}
                                items</span>
                        @endif
                    </div>
                        ...
                        <div class="space-y-2 max-h-40 overflow-y-auto">
                            @foreach ($data['checked_out_equipment'] as $loan)
                                <div
                                    ...
                                    @if ($loan['is_overdue'])
                                        <span
                                            class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20 dark:bg-red-400/10 dark:text-red-400 dark:ring-red-400/20">
